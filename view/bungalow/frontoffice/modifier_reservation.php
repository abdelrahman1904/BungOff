...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    } elseif ($date_arrive >= $date_depart) {
        $erreurs[] = "La date d'arrivée doit être avant la date de départ.";
    }

    // Contrôle du nombre de personnes
    if (!ctype_digit($nbp) || intval($nbp) <= 0) {
        $erreurs[] = "Le nombre de personnes doit être un entier positif.";
    }

    // Contrôle du prix
    if (!is_numeric($prix_total) || floatval($prix_total) <= 0) {
        $erreurs[] = "Le prix total doit être un nombre positif.";
    }
...
<script>
    document.querySelector("form").addEventListener("submit", function(e) {
        let dateArrive = document.getElementById("date_arrive").value.trim();
        let dateDepart = document.getElementById("date_depart").value.trim();
        let nbp = document.getElementById("nbp").value.trim();

        let erreurs = [];

        if (!dateArrive || !dateDepart) {
            erreurs.push("Les deux dates doivent être renseignées.");
        } else {
            let d1 = new Date(dateArrive);
            let d2 = new Date(dateDepart);
            if (isNaN(d1.getTime()) || isNaN(d2.getTime())) {
                erreurs.push("Les dates saisies ne sont pas valides.");
            } else if (d1 >= d2) {
                erreurs.push("La date d'arrivée doit être avant la date de départ.");
            }
        }

        if (!/^\d+$/.test(nbp) || parseInt(nbp) <= 0) {
            erreurs.push("Le nombre de personnes doit être un entier positif.");
        }
